...
function calculaJurosVencimento($vencimento, $valorFatura): float
{
    $taxa = 0.02;
    $jurosDia = 0.0033;
    if (!is_null($vencimento)
        ...
        $ndias = getDiferencaDias($vencimento, date('Y-m-d H:i:s'));

        if ($ndias > 0) {
            $valorTaxa = $valorFatura * $taxa;
            $valorJuros = $jurosDia * $valorFatura * $ndias;

            return ($valorTaxa + $valorJuros);
        }
    }
    return 0;
}
